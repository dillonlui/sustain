...
| 2026-09-05 | Protected 1.1.0 release attempt | `v1.1.0` | Tests, Developer ID import, notarization credentials, signed/notarized/stapled canonical app, Universal containers, and recursive artifact verification passed twice. No GitHub release or appcast was published. | First attempt exposed mismatched Sparkle environment keys; after securely synchronizing the pair, the second attempt proved `generate_appcast` signs enclosures but does not add the feed-level signature. Functional review then found that Sparkle 2.9 stores the feed signature in a trailing `sparkle-signatures` block rather than the obsolete inline attribute expected by the validator. Preserve the immutable unpublished tag; 1.1.1 build 4 signs the feed before validation and verification and validates Sparkle's actual format. |

## Manual QA deferred from 1.1.1

All implementable repository work and automated checks are complete. Product
accepted shipping 1.1.1 with the following items deliberately unchecked; none
