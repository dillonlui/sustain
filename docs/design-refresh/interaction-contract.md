...
- Reserve heights for status words, longer subdivision labels, and notices; use wrapping/truncation plus help where necessary. Keep countoff as a pinned overlay at the bottom of the performance pane so the beat remains visible even when the editor narrows the pane. Check 1200 × 700, a wider window, and editor open; idle → countoff → playing → transition → stopped must not move the shell or transport.
