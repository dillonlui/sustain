...
| --- | --- | --- |
| Which setlist occurrence is playing? | `runtime.playingEntryID` | Only this entry gets the playing edge; the main panel names it and VoiceOver announces it. A repeated occurrence of the same `Song.ID` is not playing. |
| Which occurrence will Start use? | `runtime.cuedEntryID` | Cue outline; the main panel names it and VoiceOver announces it. It may coincide with Playing. The row's cue button updates this state; native list selection is not used. |
...
| Idle with cue | Cued row outlined; NOW empty, NEXT names cue | Off, or Pad Preparing / Fading In / Playing if cued pad is pre-rolled | Start available only if validation permits; route warning/blocker stays visible. |
| Cued pad pre-roll | Still no playing edge or NOW song | Pad names selected audible/preparing pad and says “Cued pad”; Click Off | Start promotes matching pre-roll without implying song playback began earlier. |
| Starting / preparing | No playing edge until `playingEntryID` is assigned; keep the cue and the prior NOW if transitioning | Show preparing for the target channel; preserve any already audible pad identity | Start must not issue a second start; Stop remains available. Use existing message strip for failure. |
| Countoff | `playingEntryID` row receives the playing edge, but NOW/central state must say **Count in**; NEXT remains the cue, possibly the same entry | Click **Count in**; Pad follows its own preparing/fading/playing state | Numbered beat overlay remains in its fixed position; Stop available; active subdivision picker disabled. |
| Playing | Playing edge on exact entry. Cue can be on the same or a different entry; show both roles when same | Pad and Click use independent state words, not one blanket “active” state | Start disabled when cue equals playing entry; otherwise label Transition. |
...
The setlist uses a playing edge for song playback identity and a distinct cue outline. These do not imply measured audio or pad/click output. Keep explicit state words in NOW/NEXT and the row's accessibility value. If playing and cued coincide, both states remain discoverable without two competing full-row highlights.

## Component behavior

- **Performance surface:** Keep the dark square and fine edge/corner lines, with fixed positions for NOW, NEXT, countoff, and transport. Put Pad and Click in adjacent cards immediately below it, each with its status, volume setting, and action. Every status needs icon and words; color/glow only reinforces state. Preparing, fading, countoff, unavailable, and off must not look like ordinary Playing.
...
- **Rehearse:** Its selected pad is a session choice, not the Live cue. Rehearse pad and click states come from `rehearse.padState` / `rehearse.clickState`; its session subdivision is independent of song defaults. Show all pads without a search field or included-pad filter, preserve unavailable pad reasons and the two-column-to-stacked breakpoint, and keep selected versus playing appearance distinct.
...
- Use readable text for every state in the main panel and accessibility values. Do not rely on green hue, animation, or glow. Verify increased contrast and light appearance as well as dark. Honor Reduce Motion for glow and numeric countoff transitions. Announce a completed click switch or failure once, never each beat or animated frame.
- Preserve the custom pane shell. The root sidebar becomes a 64-point icon rail below 900 points; Live collapses its setlist into an expandable section below 680 points of detail width, with the editor in a sheet. Do not reintroduce `NavigationSplitView` or `.inspector`: both caused the measured playback layout jump in `13_Live_Layout_Investigation.md`.
- Allow the Live performance surface to follow its content. Show only NEXT when a cue exists without a playing song, and only NOW when playback has no cue. Keep countoff centered when active and avoid covering the transport. Check 640 × 600, 1200 × 700, a wider window, and editor open.

## Mockup corrections before implementation

1. The setlist's playing edge indicates song state, not a real meter. Pair it with explicit words in the main panel and accessibility value.
