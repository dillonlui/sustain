...
| Select and start an available pad | The open Pad readout names Preparing, Fading In, and Playing from `rehearse.padState`. The tile uses only the short `Playing` label once playback starts; only that tile gets playing treatment. The Click readout does not light merely because a pad starts. |
| Start a different pad or stop current pad | Old playing identity clears or transitions according to store truth; no two tiles claim `Playing` from selection alone. The open Pad readout carries Fading Out until the pad is actually off. Stop Pad remains available while pad activity persists. |
...
| Click off → preparing → countoff → playing | The open Click readout names each phase from `rehearse.clickState`. Countoff shows the current beat and total in the performance surface; BPM and time signature remain fixed in place. The click button reflects the action it will take, and controls remain keyboard operable. |
...
1. At **1200 × 700**, the pad and click panels sit side by side, their controls fit without clipping or overlap, and the selected/playing pad is visibly distinct from other pads by border/fill **and text**. The large top performance surface contains the detailed Pad/Click statuses. The Pads section heading has no repeated status, and routine pad progress messages are not duplicated below the main readout. Long custom labels and device names truncate only with a full help and accessibility value.
...
5. Retain Rehearse operations: pad start/stop, click start/stop, countoff, BPM slider and stepper, time signature, accent, countoff sound, subdivision, volume commits, and route/audio status. Show the included and custom pads directly, without search or a visibility checkbox. New visuals must not alter their enabled states or audio behavior.
